...
//-----------------------------------------------------------------------------
Core.register ("Context", (function () {
	var _type = Core._("Helpers.Type");
	var _event = Core._("Helpers.Event");
	var _interpreter_onchange;

	var Context = /** @lends Context.prototype */ {
		//---------------------------------------------------------------------
		/**
		 ...
		 * @type type
		 */
		execute: function (interpreter, id, parameters, nodes) {
			var rule = this.rules[id], result = false;
			var _type_reup = _type;

			if (!_type.isFunction (rule)) {
				rule = this.rules["_default"];
				if (_event.isEvent (parameters[0])) parameters[0] = id;
			}
	 ...
	 * @private
	 * @param {Interpreter} interpreter Description
	 * @param {Interpreter} oldval Description
	 * @return Description
	 * @type Interpreter
	 */
	_interpreter_onchange = function (interpreter, oldval) {
